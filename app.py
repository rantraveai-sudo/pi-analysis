...
# --- NEW DATABASE FUNCTIONS for NeonDB (PostgreSQL) ---
...
def init_db():
    ...
    This version uses SQLAlchemy's text() for compatibility and checks
    if default users need to be inserted.
    """
    ...
    with conn.session as s:
        ...
        s.execute(text('''CREATE TABLE IF NOT EXISTS scores 
                         (username TEXT, factor_id TEXT, impact REAL, performance REAL,
                          PRIMARY KEY(username, factor_id));''')) # Changed for ON CONFLICT
...
def login(username, password):
    ...
    Updated to use the new connection method and parameter style.
    """
    conn = get_db_connection()
    query = "SELECT * FROM users WHERE username = :user AND password = :pass"
    user = conn.query(sql=query, params={"user": username, "pass": password})
    if not user.empty:
...
def admin_dashboard():
            ...
            try:
                ...
                if st.button("💾 Save to Database", key="save_factors"):
                    with st.spinner("Processing... This will clear all existing scores and overrides."):
                        # Updated to use new connection and SQL syntax
                        with conn.session as s:
    ...
    with tab2:
        ...
        all_users = conn.query("SELECT username FROM users WHERE role='User'")
        submitted_users = conn.query("SELECT DISTINCT username FROM scores")
    ...
    with tab3:
        ...
        if df_scores.empty:
            ...
            overrides_df = conn.query("SELECT factor_id, override_rank FROM rank_overrides")
            ...
            st.dataframe(df_agg[list(display_cols.keys())].rename(columns=display_cols))
    ...
    with tab4:
        ...
        df_scores_tab4 = conn.query(df_scores_query)
        ...
        if not df_scores_tab4.empty:
            ...
            if not tied_scores.empty:
                ...
                st.dataframe(tied_scores[['Initial_Rank', 'factor_id', 'factor_text', 'Priority_Score']])
...
def user_dashboard():
    ...
    conn = get_db_connection()
    df_factors = conn.query("SELECT * FROM factors ORDER BY factor_id;")
